...
import { Category, MenuItemFormData, AddMenuItemModalProps } from '@/api/models';
...
export default function AddMenuItemModal({ isOpen, onClose, categories, onSubmit, item }: AddMenuItemModalProps) {
 ...
 id: item.id,
 name: item.name || '',
 category: item.category || (categories.length > 0 ? categories[0].id : 0),
 ...
 alias: item.alias || '',
 quantity_if_package: item.quantity_if_package,
 price_if_package: item.price_if_package,
 });
 ...
 // Set default category when categories are loaded (only if not in edit mode)
 useEffect(() => {
 if (categories.length > 0 && formData.category === 0 && !isEditMode) {
 ...
 <h2 className="text-2xl font-bold">{isEditMode ? 'Edit Menu Item' : 'Add Menu Item'}</h2>
 <button
 onClick={handleClose}
 ...
 {formData.type === 'PACKAGES' && (
 <div className="mb-8">
 <h3 className="text-lg font-semibold mb-4 text-primary">Package Information</h3>
 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
 {/* Quantity if Package */}
 <div>
